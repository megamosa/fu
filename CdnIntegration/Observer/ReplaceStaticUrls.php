...
class ReplaceStaticUrls implements ObserverInterface
{
    ...
    private function processUrl($html, $url, $cdnBaseUrl, $baseUrl, $secureBaseUrl, $safeFileTypes, $criticalFiles, &$replacementCount, &$replacedUrls)
    {
        try {
            ...
            // Determine path to use in CDN URL - moved here to make $cdnUrl available for all code paths
            $cdnPath = '';
...
// Special handling for RequireJS modules
                       ...
                       $html = preg_replace_callback(
                           ...
                           function($matches) use ($cdnUrl, &$replacementCount, $normalizedUrl) {
                               ...
                               if (strpos($matches[4], '/static/') === 0 || strpos($matches[4], '/media/') === 0) {
                                   $replacementCount++;
                                   return $matches[1] . $module . $matches[2] . $matches[3] . $cdnUrl . $matches[5];
                               }
                               return $matches[0];
                           },
                           $html
           ...
           // 5. Special handling for RequireJS data-requiremodule attribute
           $pattern = '/(\sdata-requiremodule=["\'])(' . preg_quote($normalizedUrl, '/') . ')(["\'])/';
           $html = preg_replace_callback(
               $pattern,
               function($matches) use ($cdnUrl, &$replacementCount) {
                   $replacementCount++;
           ...
           // 6. Special handling for require.js text plugin
           $textPluginPattern = '/text!(["\'])(' . preg_quote($normalizedUrl, '/') . ')(["\'])/';
           $html = preg_replace_callback(
               $textPluginPattern,
               function($matches) use ($cdnUrl, &$replacementCount) {
                   $replacementCount++;
                   return 'text!' . $matches[1] . $cdnUrl . $matches[3];
               },
               $html
           );
           
           // 7. Special handling for JSON data in data attributes
           if (strpos($normalizedUrl, '.js') !== false || strpos($normalizedUrl, '.css') !== false) {
               // Corregido: No usar str_replace en combinación con preg_quote
               $jsonPattern = '/"' . preg_quote($normalizedUrl, '/') . '"/';
               $html = preg_replace_callback(
                   $jsonPattern,
                   function($matches) use ($cdnUrl, &$replacementCount) {
                       $replacementCount++;
           ...
           // 8. Special handling for Magento's mage/utils and related files for RequireJS
           if (strpos($normalizedUrl, '/mage/') !== false || 
               strpos($normalizedUrl, '/Smartwave/') !== false ||
               strpos($normalizedUrl, '.min.js') !== false) {
               
               // Look for RequireJS define calls that include this module
               $definePattern = '/define\s*\(\s*[\'"]((?:[^\'"]|\\\\[\'"])*?)[\'"]\s*,\s*\[([^\]]*?)(' . 
                                 preg_quote($normalizedUrl, '/') . '|' . 
                                 preg_quote(str_replace('/static/', '', $normalizedUrl), '/') . 
                                 ')\s*([^\]]*?)\]/';
               ...
               $html = preg_replace_callback(
                   ...
                   function($matches) use ($cdnUrl, &$replacementCount, $normalizedUrl) {
                       $replacementCount++;
                       // Keep the original format but replace just the URL
                       return str_replace($normalizedUrl, $cdnUrl, $matches[0]);
                   },
                   $html
               );
           }
